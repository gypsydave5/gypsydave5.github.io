...
## Example: Where does authentication go?

All of the above is abstract. Let's make it earn its keep on a problem everyone has to solve and nearly everyone puts in the wrong place: authentication.
...
**The identity itself is a domain value.** The _result_ of authenticating is a small fact: "this call is being made by this user, with these claims". That's a domain type - a `Principal`, a `UserId`, call it what you like. It isn't a token and it isn't a header. It's a value, and like every other domain value it travels _inward_, as part of the command or query the use case receives.

**Turning a credential into that identity is adaptor work.** Parsing the `Authorization` header, checking a JWT's signature, reading a session cookie - that's transport plumbing, and transport plumbing is what an in-adaptor is _for_. The HTTP adaptor already turns a request into a use-case call; working out _who_ is making the call is just part of that same translation. (Any signing keys it needs come from `Bootstrap`, like all configuration.)

**If verifying the credential means hitting something, that something is an out-port.** A session store, a users table, a call out to an identity provider - these are out-ports like any other, faked and contract-tested exactly as we've described. If your scheme is stateless - a JWT you can verify with a key alone - there's no out-port at all; it stays in the adaptor.

**And authorization - "is this principal _allowed_ to do this?" - goes inside the use case.** This is the one people get wrong, and it's the one this whole approach refuses to let you fudge. Whether a user may perform an action is a _business rule_. It is not a property of HTTP. It belongs below the in-port, in the application, next to the logic it protects.
...
|---|---|---|
| The `Principal` / identity type | domain | it's just an argument - pass whichever identity you like |
...
And look what it buys you at test time. Because the `Principal` is just an input to the use case, testing authorization needs no tokens, no HTTP, no fixtures: you drive the in-port directly with an admin, an anonymous user, the _wrong_ user, and assert on what happens. Fast, readable, and in the application's own language.
...
### When getting the identity needs a round-trip

There's a wrinkle in all that. I said the in-adaptor turns the credential into a `Principal` - which is fine when it can do that on its own, verifying a JWT with a key it already holds. But often it can't. An opaque token has to be introspected; a session id has to be looked up; a cookie has to be exchanged with an identity provider. Now producing the identity needs _I/O_ - and I/O means an out-port, and adaptors have no business _consuming_ out-ports. That's the application's job.

The way out is to notice that this, too, is just a use case. Authentication that needs a round-trip is an in-port like any other:

- an `IdentifyUser` use case takes the raw credential - the token or session id - and returns a `Principal`, or fails;
- its implementation is a query handler that uses an out-port - a `TokenIntrospector`, an `IdentityProvider`, a `SessionStore` - to do the exchange;
- and the HTTP adaptor runs it as _middleware_: credential in, `IdentifyUser`, `Principal` out, and only _then_ the business use case, with that principal in hand.
...
Which means a protected request runs through _two_ in-adaptors, and that's exactly right. First the auth middleware adapts the request into `IdentifyUser` and comes back with a `Principal` (or, on failure, translates that into a 401 - how a rejection looks over HTTP is transport, not business). Then the route handler adapts the request into the business use case, folding in the `Principal` the middleware just established. Two adaptors, two use cases, one request.
