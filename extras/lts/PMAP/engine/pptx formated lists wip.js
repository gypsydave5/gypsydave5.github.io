...
function preProcessSlide(){
  const maths = document.querySelectorAll(".katex .katex-mathml math");
  for (var i = 0, len = maths.length; i < len; i++) {
    var math = maths[i];
    let mathStyle = window.getComputedStyle(math, null);
    let mathColor = mathStyle.getPropertyValue('color');
    let convertedMathColor = toPptxColor(mathColor)[0];
    math.dataset.equationid = i;
    math.dataset.mathcolor = convertedMathColor;
  }
}

function processSlide(equations, escapedNotes, firstSlide) {
...
function getTextOptions(textStyle) {
  ...
  let fontSize = textStyle.getPropertyValue('font-size').slice(0, -2) * 0.75 * 0.56; // remove the px suffix, 0.75 = conversion between px to pt
...
function processText(element, options) {
  ...
  // Maths?
  if (element.querySelectorAll(".katex-rendered").length > 0)
  {
      const maths = document.querySelectorAll(".katex-rendered");
      for (var i = 0, len = maths.length; i < len; i++) {
        var math = maths[i];
        processMath(math);
      }
...
function processMath(element){
    let innerMathElement = element.querySelector(".katex-mathml math");
    let equationId = innerMathElement.dataset.equationid;
    var options = getPositionAnsSizeOptions(element);
    var processedMath = window.equations[equationId];
    options.math = processedMath;
    window.slide.addMath(options);
}
...
function flattenList(listRoot, level = 0, out = []) {
  const currentListType = listRoot.nodeName;

  for (const li of listRoot.children) {
    if (li.nodeName !== 'LI') continue;

    let textParts = [];

    // Collect inline text ONLY — stop at nested lists
    for (const node of li.childNodes) {
      if (node.nodeType === Node.TEXT_NODE) {
        const [text, options] = sanitizeText(node.textContent);
        if (text.trim()) textParts.push({ text, options });
      } else if (
        node.nodeType === Node.ELEMENT_NODE &&
        !['UL', 'OL'].includes(node.nodeName)
      ) {
        const style = window.getComputedStyle(node, null);
        const inlineOptions = getTextOptions(style);
        processInnerText(node, textParts, inlineOptions);
      }
    }

    if (textParts.length > 0) {
      out.push({ textParts, indentLevel: level, listType: currentListType });
    }

    // Now process nested lists
    for (const node of li.childNodes) {
      if (node.nodeType === Node.ELEMENT_NODE && (node.nodeName === 'UL' || node.nodeName === 'OL')) {
        flattenList(node, level + 1, out); // Pass real list tag
      }
    }
  }
...
function renderFlattenedList(listRoot) {
  const items = flattenList(listRoot);
  const textParts = [];
  ...
  for (const item of items) {
    ...
    if (item.listType === 'UL') {
      bulletOption = true;
    } else if (item.listType === 'OL') {
      bulletOption = {
        type: 'number',
        style: numberStyles[item.indentLevel] || numberStyles[numberStyles.length - 1]
      };
    }

    let firstApplied = false;
    item.textParts.forEach(part => {
      const options = Object.assign({}, part.options);
      if (!firstApplied && part.text.trim() !== "") {
        options.bullet = bulletOption;
        options.indentLevel = item.indentLevel;
        firstApplied = true;
      }
      textParts.push({ text: part.text, options });
    });

    // Edge case: if no text at all, still push a placeholder
    /*if (!firstApplied) {
      textParts.push({
        text: " ",
        options: {
  ...
  const positionOptions = getPositionAnsSizeOptions(listRoot);
  const boxOptions = getBoxOptions(window.getComputedStyle(listRoot));
...
const safeTextParts = structuredClone(textParts);
console.log("window.slide.addText(", safeTextParts, ",", finalOptions, ")");

  window.slide.addText(textParts, {
    fit: 'shrink',
    ...positionOptions,
    ...boxOptions
  });
